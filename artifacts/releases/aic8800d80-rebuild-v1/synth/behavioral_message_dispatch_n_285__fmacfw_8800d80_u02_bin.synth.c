/* task=behavioral_message_dispatch_n_285 class=behavioral priority=critical score=25.000 */
/* image=fmacfw_8800d80_u02_bin addr=0x122c706 */

void message_dispatch_n_285(void) {
  /* reconstructed_micro_flow: yes */
    *((volatile uint32_t *)(uintptr_t)0x0U) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0x4U) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0x8U) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0xcU) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0x10U) = 0xb8U;
    *((volatile uint32_t *)(uintptr_t)0x14U) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0x18U) = 0x10U;
    *((volatile uint32_t *)(uintptr_t)0x1cU) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0x2007fee8U) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0x2007feecU) = 0xb8U;
    *((volatile uint32_t *)(uintptr_t)0x2007fef0U) = 0x60U;
    *((volatile uint32_t *)(uintptr_t)0x2007fef4U) = 0x122cb1cU;
    *((volatile uint32_t *)(uintptr_t)0x2007fef8U) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0x2007fefcU) = 0xdeadc001U;
}
