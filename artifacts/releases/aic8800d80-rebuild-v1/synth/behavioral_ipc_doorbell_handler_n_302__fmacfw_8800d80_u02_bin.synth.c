/* task=behavioral_ipc_doorbell_handler_n_302 class=behavioral priority=critical score=25.000 */
/* image=fmacfw_8800d80_u02_bin addr=0x121567a */

void ipc_doorbell_handler_n_302(void) {
  /* reconstructed_micro_flow: yes */
    (void)*((volatile uint32_t *)(uintptr_t)0x173199U);
    (void)*((volatile uint32_t *)(uintptr_t)0x40328048U);
    *((volatile uint32_t *)(uintptr_t)0x40328048U) = 0x0U;
    (void)*((volatile uint32_t *)(uintptr_t)0x40328048U);
    *((volatile uint32_t *)(uintptr_t)0x40328048U) = 0x0U;
    (void)*((volatile uint32_t *)(uintptr_t)0x182ba0U);
    (void)*((volatile uint32_t *)(uintptr_t)0x0U);
    (void)*((volatile uint32_t *)(uintptr_t)0x182b78U);
    (void)*((volatile uint32_t *)(uintptr_t)0x40500038U);
    *((volatile uint32_t *)(uintptr_t)0x40500038U) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0x2007fef4U) = 0x1676fcU;
    *((volatile uint32_t *)(uintptr_t)0x2007fef8U) = 0x40500000U;
    *((volatile uint32_t *)(uintptr_t)0x2007fefcU) = 0x0U;
    *((volatile uint32_t *)(uintptr_t)0x2007fef0U) = 0x12156c7U;
    (void)*((volatile uint32_t *)(uintptr_t)0x182b60U);
    (void)*((volatile uint32_t *)(uintptr_t)0x0U);
    (void)*((volatile uint32_t *)(uintptr_t)0x2007fef0U);
    (void)*((volatile uint32_t *)(uintptr_t)0x182b9cU);
    (void)*((volatile uint32_t *)(uintptr_t)0x0U);
    (void)*((volatile uint32_t *)(uintptr_t)0x173199U);
    (void)*((volatile uint32_t *)(uintptr_t)0x0U);
    *((volatile uint32_t *)(uintptr_t)0x40320038U) = 0x0U;
    (void)*((volatile uint32_t *)(uintptr_t)0x0U);
    (void)*((volatile uint32_t *)(uintptr_t)0x182ba0U);
    (void)*((volatile uint32_t *)(uintptr_t)0x0U);
    (void)*((volatile uint32_t *)(uintptr_t)0x2007ff00U);
    (void)*((volatile uint32_t *)(uintptr_t)0x2007ff04U);
    (void)*((volatile uint32_t *)(uintptr_t)0x2007ff08U);
    (void)*((volatile uint32_t *)(uintptr_t)0x2007ff0cU);
    (void)*((volatile uint32_t *)(uintptr_t)0x2007ff10U);
    (void)*((volatile uint32_t *)(uintptr_t)0x2007ff14U);
}
