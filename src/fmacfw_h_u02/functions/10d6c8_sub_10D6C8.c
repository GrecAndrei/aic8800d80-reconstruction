#include <stdint.h>
#include <stddef.h>
#include <stdarg.h>
#include <inttypes.h>

#define LOBYTE(x) ((uint8_t)((x) & 0xFF))
#define HIBYTE(x) ((uint8_t)(((x) >> 8) & 0xFF))
#define LOWORD(x) ((uint16_t)((x) & 0xFFFF))
#define HIWORD(x) ((uint16_t)(((x) >> 16) & 0xFFFF))
#define LODWORD(x) ((uint32_t)(x))
#define HIDWORD(x) ((uint32_t)(((uint64_t)(x) >> 32)))

extern uint32_t off_10D994;
extern uint32_t off_10D998;
extern uint32_t off_10D99C;
extern uint32_t off_10D9A0;
extern uint32_t off_10D9A4;
extern uint32_t off_10D9A8;
extern uint32_t off_10D9AC;
extern uint32_t off_10D9B4;
extern uint32_t dword_10D9B0;
extern uint32_t dword_10D9B8;
extern uint32_t dword_10D9E4;
extern uint32_t dword_10D9E0;
extern uint32_t dword_10D9E8;
extern uint32_t off_10D9EC;
extern uint32_t dword_10DAC0;
extern uint32_t off_10DAC4;
extern uint32_t off_10DADC;
extern uint32_t off_10DAE0;
extern uint32_t off_10DAC8;
extern uint32_t off_10DACC;
extern uint32_t off_10DAD0;
extern uint32_t off_10D9F0;
extern uint32_t dword_10D9BC;
extern uint32_t dword_10D9C0;
extern uint32_t off_10D9F4;
extern uint32_t dword_10D9C4;
extern uint32_t off_10D9C8;
extern uint32_t dword_10DAD4;
extern uint32_t off_10D9CC;
extern uint32_t off_10D9D0;
extern uint32_t off_10D9D4;
extern uint32_t off_10D9D8;
extern uint32_t dword_10D9DC;
extern uint32_t dword_10DAD8;

// stack_struct_init @ 0x10d6c8, size 1016 bytes
int  stack_struct_init(int a1, int a2)
{
  int v2; // r4
  unsigned int CPSR; // r6
  int v4; // r0
  int v5; // r1
  int v6; // r7
  int v7; // r5
  int v8; // r3
  uint8_t *v9; // r4
  uint32_t *v10; // r1
  uint32_t *v11; // r0
  uint8_t *v12; // r5
  int v13; // r2
  int v14; // t1
  int v16; // r3
  uint8_t **v17; // r8
  int v18; // r0
  uint32_t *v19; // r2
  int *v20; // r4
  int v21; // r1
  int v22; // t1
  int v23; // r1
  uint8_t *v24; // r8
  int v25; // r9
  uint8_t *v26; // r0
  uint8_t *v27; // r8
  uint8_t *v28; // r9
  int v29; // r0
  uint32_t *v30; // r2
  int *v31; // r12
  int v32; // r1
  int v33; // t1
  uint16_t *v34; // r10
  int v35; // r11
  int v36; // r0
  uint32_t *v37; // r2
  int *v38; // r5
  int v39; // r1
  int v40; // t1
  int *v41; // r3
  int v42; // r0
  int v43; // r8
  int v44; // r3
  uint8_t *v45; // r1
  unsigned int v46; // r2
  int16_t v47; // r0
  uint32_t *v48; // r5
  int v49; // r2
  int *v50; // r5
  int v51; // r0
  int v52; // r0
  int v53; // r3
  int v54; // r2
  int v55; // r0
  uint32_t *v56; // r2
  int *v57; // r4
  int v58; // r1
  int v59; // t1
  int v60; // r0
  uint32_t *v61; // r2
  int *v62; // r4
  int v63; // r1
  int v64; // t1
  int v65; // r0
  uint32_t *v66; // r2
  int *v67; // r12
  int v68; // r1
  int v69; // t1
  int v70; // r2
  uint64_t v71; // r0
  int ( *v72)(uint32_t); // r2
  int v73; // r0
  uint16_t *v74; // r2
  int v75; // r2
  uint16_t v76; // r5
  int v77; // r1
  uint32_t *v78; // r2
  int *v79; // r0
  int v80; // t1
  int *v81; // [sp+Ch] [bp-108h]
  uint8_t v82[260]; // [sp+10h] [bp-104h] BYREF

  CPSR = __get_CPSR();
  v4 = rx_process_packet(v82, 256, 0, a1, a2);
  v6 = v4;
  if ( v4 <= 0 )
    return v6;
  v7 = v4;
  if ( *(uint32_t *)off_10D994 )
  {
    if ( CPSR )
      goto LABEL_5;
  }
  else
  {
    kernel_schedule_check();
    if ( CPSR )
      goto LABEL_5;
  }
  while ( !*(uint32_t *)off_10D998 )
    ;
LABEL_5:
  if ( !*(uint32_t *)off_10D99C )
  {
    do
      v5 = *(uint32_t *)off_10D9A0 << 12;
    while ( (*(uint32_t *)off_10D9A0 & 0x80000) != 0 );
    *(uint32_t *)off_10D9A4 = 95;
  }
  v8 = *((uint16_t *)off_10D9A8 + 89) & 0x4000;
  if ( (*((uint16_t *)off_10D9A8 + 89) & 0x4000) == 0 )
  {
    v9 = v82;
    v10 = off_10D9A0;
    v11 = off_10D9A4;
    v12 = &v82[v7];
    do
    {
      v14 = *v9++;
      v13 = v14;
      if ( v14 == 10 && v8 != 13 )
      {
        while ( (*v10 & 0x80000) != 0 )
          ;
        *v11 = 13;
        v13 = *(v9 - 1);
      }
      while ( (*v10 & 0x80000) != 0 )
        ;
      *v11 = v13;
      v8 = *(v9 - 1);
    }
    while ( v9 != v12 );
    goto LABEL_13;
  }
  v16 = **(uint8_t **)off_10D9AC;
  if ( v16 == 1 )
  {
    if ( **(uint8_t **)off_10D9B4 != 3 )
    {
      if ( v7 >= 124 )
        LOWORD(v7) = 124;
      kmalloc(19, v82, (uint16_t)v7);
    }
    goto LABEL_13;
  }
  if ( v16 != 2 )
    goto LABEL_13;
  v17 = (uint8_t **)off_10D9B4;
  if ( **(uint8_t **)off_10D9B4 == 3 )
  {
LABEL_19:
    v18 = dword_10D9B0;
    v19 = off_10D9A0;
    v20 = (int *)off_10D9A4;
    v21 = 110;
    do
    {
      while ( (*v19 & 0x80000) != 0 )
        ;
      *v20 = v21;
      v22 = *(uint8_t *)++v18;
      v21 = v22;
    }
    while ( v22 );
    goto LABEL_13;
  }
  if ( (unsigned int)list_length(dword_10D9B8, v5) <= 4 )
  {
    if ( **(uint8_t **)off_10D9AC == 2 )
    {
      if ( **v17 != 3 )
      {
        if ( (unsigned int)list_length(dword_10D9B8, v23) > 4 )
        {
          v60 = dword_10D9E4;
          v61 = off_10D9A0;
          v62 = (int *)off_10D9A4;
          v63 = 111;
          do
          {
            while ( (*v61 & 0x80000) != 0 )
              ;
            *v62 = v63;
            v64 = *(uint8_t *)++v60;
            v63 = v64;
          }
          while ( v64 );
        }
        else
        {
          v55 = dword_10D9E0;
          v56 = off_10D9A0;
          v57 = (int *)off_10D9A4;
          v58 = 102;
          do
          {
            while ( (*v56 & 0x80000) != 0 )
              ;
            *v57 = v58;
            v59 = *(uint8_t *)++v55;
            v58 = v59;
          }
          while ( v59 );
        }
        goto LABEL_13;
      }
      goto LABEL_19;
    }
LABEL_13:
    if ( !CPSR )
      *(uint32_t *)off_10D998 = 1;
    return v6;
  }
  v24 = v82;
  v25 = dword_10D9E8;
  while ( 1 )
  {
    v26 = v24++;
    if ( !memcmp(v26, v25, 20) )
      break;
    if ( &v82[v7] == v24 )
    {
      v27 = off_10D9EC;
      goto LABEL_37;
    }
  }
  v65 = dword_10DAC0;
  v66 = off_10DAC4;
  v67 = (int *)off_10DADC;
  v68 = 70;
  do
  {
    while ( (*v66 & 0x80000) != 0 )
      ;
    *v67 = v68;
    v69 = *(uint8_t *)++v65;
    v68 = v69;
  }
  while ( v69 );
  v27 = off_10DAE0;
  v70 = *(uint32_t *)off_10DAC8;
  v81 = (int *)off_10DACC;
  v71 = *(uint64_t *)(v70 + 4);
  v72 = *(int ( **)(uint32_t))(HIDWORD(v71) + 16);
  *(uint8_t *)off_10DAE0 = 1;
  v73 = v72(v71);
  v74 = off_10DAD0;
  *v81 = v73;
  *v74 = 4;
LABEL_37:
  if ( *v27 )
  {
    v28 = off_10D9F0;
    if ( !memcmp(v82, dword_10D9BC, 5) )
    {
      v29 = dword_10D9C0;
      v30 = off_10D9A0;
      v31 = (int *)off_10D9A4;
      *v28 = 1;
      v32 = 80;
      do
      {
        while ( (*v30 & 0x80000) != 0 )
          ;
        *v31 = v32;
        v33 = *(uint8_t *)++v29;
        v32 = v33;
      }
      while ( v33 );
    }
    v34 = (uint16_t *)off_10D9F4;
    v35 = *(uint16_t *)off_10D9F4;
    if ( v35 + v7 <= 1720 )
    {
      v81 = (int *)off_10DACC;
      memcpy(*(uint32_t *)off_10DACC + v35, v82, v7);
      v75 = (uint8_t)*v28;
      v41 = v81;
      v76 = v7 + *v34;
      v35 = v76;
      *v34 = v76;
      if ( !v75 )
        return v6;
    }
    else
    {
      v36 = dword_10D9C4;
      v37 = off_10D9A0;
      v38 = (int *)off_10D9A4;
      v39 = 66;
      do
      {
        while ( (*v37 & 0x80000) != 0 )
          ;
        *v38 = v39;
        v40 = *(uint8_t *)++v36;
        v39 = v40;
      }
      while ( v40 );
      v41 = (int *)off_10D9C8;
    }
    v42 = dword_10D9B8;
    *v27 = 0;
    *v28 = 0;
    v43 = *v41;
    *v34 = 4;
    *v41 = 0;
    v6 = v35;
    list_pop(v42);
    goto LABEL_48;
  }
  v2 = list_pop(dword_10DAD4);
  if ( v7 <= 122 )
    v43 = util_call_void();
  else
    v43 = (*(int ( **)(uint32_t))(*(uint32_t *)(*(uint32_t *)off_10DAC8 + 8) + 16))(*(uint32_t *)(*(uint32_t *)off_10DAC8
                                                                                                  + 4));
  if ( v43 )
  {
    memcpy(v43 + 4, v82, v7);
    LOWORD(v35) = v7;
LABEL_48:
    *(uint16_t *)v43 = v6 + 1;
    *(uint8_t *)(v43 + 2) = 19;
    v44 = 0;
    v45 = off_10D9CC;
    *(uint8_t *)(v43 + 3) = 0;
    *(uint8_t *)(v43 + v6 + 4) = 0;
    v46 = *((uint16_t *)v45 + 4122);
    if ( v46 > 0x186 )
    {
      LOWORD(v46) = 0;
      v47 = 1;
    }
    else
    {
      v47 = v46 + 1;
      v44 = 8 * v46;
    }
    v48 = off_10D9D0;
    *(uint16_t *)(v2 + 12) = v46;
    v49 = *v48 + v44;
    *(uint32_t *)(v49 + 4) = v43;
    *(uint16_t *)v49 = v35 + 5;
    *(uint8_t *)(v49 + 3) = *(uint8_t *)(v49 + 3) & 0x31 | 4;
    LOBYTE(v48) = v45[3074] + 1;
    *((uint16_t *)v45 + 4122) = v47;
    v45[3074] = (uint8_t)v48;
    *(uint32_t *)(v2 + 4) = v49;
    *(uint8_t *)(v2 + 14) = 1;
    *(uint32_t *)v2 = 0;
    *(uint32_t *)(v2 + 8) = v6 + 5;
    if ( (__get_CPSR() & 1) == 0 )
    {
      __disable_irq();
      *(uint32_t *)off_10D9D4 = 1;
    }
    v50 = (int *)off_10D9D8;
    v51 = dword_10D9DC;
    ++*(uint32_t *)off_10D9D8;
    v52 = wlan_ioctl_handler_1(v51);
    rf_is_idle(v52);
    if ( *v50 )
    {
      v53 = *v50 - 1;
      v54 = *(uint32_t *)off_10D9D4;
      *v50 = v53;
      if ( !v53 )
      {
        if ( v54 )
          __enable_irq();
      }
    }
    goto LABEL_13;
  }
  v77 = dword_10DAD8;
  v78 = off_10DAC4;
  v79 = (int *)off_10DADC;
  v6 = 110;
  do
  {
    while ( (*v78 & 0x80000) != 0 )
      ;
    *v79 = v6;
    v80 = *(uint8_t *)++v77;
    v6 = v80;
  }
  while ( v80 );
  return v6;
}

