...
// Doc: llc_evt_dispatch [scan]: Main scan/state task handler iterating per-station entries
// llc_evt_dispatch [scan]: Main scan/state task handler iterating per-station entries
int  llc_evt_dispatch(int a1, int a2, char a3, char a4, unsigned int a5, int a6, int a7)
{
  ...
  v7 = dword_12A6FC;
  v8 = *(uint8_t *)(a1 + 1225);
  v9 = (uint8_t *)(dword_12A6FC + 140 * v8);
  if ( v9[112] != 1 )
    return 255;
  v10 = v9[16];
  v11 = (uint8_t *)(dword_12A6FC + 140 * v8);
  if ( v9[16] )
  {
    if ( v9[64] )
      return 255;
