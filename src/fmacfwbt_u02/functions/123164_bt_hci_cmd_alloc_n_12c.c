...
// Doc: send_hci_reset_cmd [bt]: Allocate and initialize an HCI command buffer (tag 0x74)
// send_hci_reset_cmd [bt]: Allocate and initialize an HCI command buffer (tag 0x74)
int  send_hci_reset_cmd(int a1, int a2, int a3, int a4)
{
  ...
  *v4 = HIBYTE(v5);
  ...
  v7 = dword_1231E0;
  v4[4] = BYTE1(v6);
  ke_event_schedule(v7, HIBYTE(v5), (uint8_t)v6);
  hci_evt_send(v4);
  return 0;
}
