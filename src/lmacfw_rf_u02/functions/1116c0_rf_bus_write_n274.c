...
int  ke_send_event(uint32_t *a1, int a2)
{
  ...
  dispatch_event_handler(rf_bus_setup_n_3c, 0);
  if ( !a1 )
    return 1;
  ...
  *(uint8_t *)rf_bus_write_17cc = 1;
  memcpy_large(v6, a1, 80);
  v7 = v5[97];
  if ( v7 )
  {
    dispatch_event_handler(rf_bus_write2_n2dc, v7);
    write_timer_reg(v5[97]);
  }
  v8 = off_1117D8;
  ...
  dispatch_event_handler(dword_1117DC, v9);
  ...
  *(uint32_t *)(*((uint32_t *)off_1117E0 + 2) + 320) = rf_bus_reset2_17e4;
  ...
  dispatch_event_handler(rf_bus_reset2_n4e8, v10);
  return 0;
}
