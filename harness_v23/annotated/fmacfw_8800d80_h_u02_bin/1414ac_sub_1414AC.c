// v23 annotated: sub_1414AC @ 0x1414ac
// Original: 1414ac_sub_1414AC.c
// Primary struct: <unclustered>
//
// sub_1414AC @ 0x1414ac, size 186 bytes
int sub_1414AC()
{
  __int16 **v0; // r8
  int v1; // r5
  int v2; // r4
  int v3; // r6
  int v4; // r7
  int result; // r0

  v0 = (__int16 **)off_14157C;
  v1 = dword_141568;
  v2 = *(_DWORD *)(dword_141568 + 6480);
  if ( **(__int16 **)off_14157C >= 0 || v2 < 0 && (result = sub_12F32C(dword_141578, dword_14156C, 1734), **v0 >= 0) )
  {
    v3 = 16 * v2;
    if ( *(unsigned __int8 *)(v1 + 136 * v2 + 6339) > 1u )
      return result;
    return sub_140AAC(v1 + 8 * (v2 + v3) + 6336, 0);
  }
  v4 = v1 + 136 * v2;
  v3 = 16 * v2;
  if ( *(_BYTE *)(v4 + 6338) == 5 )
  {
    if ( *(unsigned __int8 *)(v4 + 6339) <= 1u )
      return sub_140AAC(v1 + 8 * (v2 + v3) + 6336, 0);
    return sub_12F32C(dword_141574, dword_14156C, 1744);
  }
  result = sub_12F32C(dword_141570, dword_14156C, 1735);
  if ( *(unsigned __int8 *)(v4 + 6339) <= 1u )
    return sub_140AAC(v1 + 8 * (v2 + v3) + 6336, 0);
  if ( **v0 < 0 )
    return sub_12F32C(dword_141574, dword_14156C, 1744);
  return result;
}

