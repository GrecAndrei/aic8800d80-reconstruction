// v23 annotated: sub_121D44 @ 0x121d44
// Original: 121d44_sub_121D44.c
// Primary struct: <unclustered>
//
// sub_121D44 @ 0x121d44, size 230 bytes
int sub_121D44()
{
  int *v0; // r8
  int *v1; // r4
  _DWORD *v2; // r5
  int v3; // r3
  int v4; // r7
  int v5; // r9
  _DWORD *v6; // r0
  _DWORD *v7; // r1
  int v8; // r4
  int v9; // r3
  int result; // r0
  int v11; // r3
  int v12; // r3
  int v13; // r2

  if ( **(__int16 **)off_121E2C < 0 && sub_12D0B0(0) == 4 )
    sub_12F694(dword_121E60, dword_121E5C, 1799);
  sub_12EEF8(4, dword_121E30);
  if ( (__get_CPSR() & 1) == 0 )
  {
    __disable_irq();
    *(_DWORD *)off_121E34 = 1;
  }
  v0 = (int *)off_121E64;
  v1 = (int *)off_121E38;
  v2 = off_121E3C;
  ++*(_DWORD *)off_121E64;
  v3 = *v1;
  *v1 = 0;
  v4 = v2[4];
  v5 = v3 & 0xF;
  while ( *v1 << 28 )
  {
    sub_117E34();
    if ( (unsigned int)(v2[4] - v4) > 0x7530 )
      sub_12EEF8(4, dword_121E58);
  }
  v6 = off_121E44;
  v7 = off_121E48;
  v8 = *(_DWORD *)off_121E44;
  *((_DWORD *)off_121E40 + 1) &= ~4u;
  if ( v5 && (*v7 & v8 & 4) == 0 )
  {
    while ( (*v7 & *v6 & 4) == 0 )
      ;
  }
  v9 = *((unsigned __int8 *)off_121E4C + 90);
  *(_DWORD *)off_121E50 = 4;
  if ( v9 )
    sub_117E64();
  result = sub_12CF5C(0);
  v11 = *v0;
  *((_WORD *)off_121E54 + 9) = 0;
  if ( v11 )
  {
    v12 = v11 - 1;
    v13 = *(_DWORD *)off_121E34;
    *v0 = v12;
    if ( !v12 )
    {
      if ( v13 )
        __enable_irq();
    }
  }
  return result;
}

