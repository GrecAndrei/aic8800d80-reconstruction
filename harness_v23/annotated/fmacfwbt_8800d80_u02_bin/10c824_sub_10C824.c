// v23 annotated: sub_10C824 @ 0x10c824
// Original: 10c824_sub_10C824.c
// Primary struct: mmio_clock_gate (cluster 1, 23 funcs)
// Fields: reg_addr=0x0, bit_idx=0x0, mask=0x18, value=0x1c
//
// sub_10C824 @ 0x10c824, size 236 bytes
int __fastcall sub_10C824(char a1)
{
  int v1; // r5
  unsigned __int8 *v2; // r11
  int v3; // r8
  int *v4; // r7
  _DWORD *v5; // r10
  unsigned int v6; // r1
  unsigned int v7; // r2
  unsigned int v8; // r4
  int *v9; // r9
  int v10; // r0
  int v11; // r1
  int v12; // r3
  int v13; // r1
  int v14; // r2
  int v15; // r0
  int result; // r0

  sub_12D374(2048);
  v1 = *(_DWORD *)off_10C910;
  if ( *(_DWORD *)(*(_DWORD *)off_10C910 + 572) )
  {
    v2 = (unsigned __int8 *)off_10C928;
    v3 = dword_10C92C;
    v4 = (int *)off_10C914;
    v5 = off_10C930;
    while ( 1 )
    {
      v6 = v2[1621];
      v7 = v2[1622];
      if ( v6 >= v7 )
        break;
      v8 = v2[1620]
         + 1
         - 81 * ((unsigned int)(((unsigned int)v3 * (unsigned __int64)((unsigned int)v2[1620] + 1)) >> 32) >> 6);
      v2[1621] = v6 + 1;
      v2[1620] = v8;
      if ( (__get_CPSR() & 1) == 0 )
      {
        __disable_irq();
        *v4 = 1;
      }
      v9 = (int *)off_10C934;
      ++*(_DWORD *)off_10C934;
      while ( !*v5 )
        ;
      v10 = sub_12D4F8(v1 + 572);
      v11 = *v9;
      *v5 = 1;
      v12 = v10;
      if ( v11 )
      {
        v13 = v11 - 1;
        v14 = *v4;
        *v9 = v13;
        if ( !v13 )
        {
          if ( v14 )
            __enable_irq();
        }
      }
      v15 = dword_10C91C;
      *(_DWORD *)&v2[20 * v8 + 4] = v12 + *(_DWORD *)off_10C918;
      list_push_tail(v15);
      if ( !*(_DWORD *)(v1 + 572) )
        goto LABEL_13;
    }
    log_printf(dword_10C924, v6, v7);
  }
LABEL_13:
  result = irq_nesting_or(256);
  *(_DWORD *)off_10C920 = (1 << (a1 + 8)) & 0x1F00;
  return result;
}

