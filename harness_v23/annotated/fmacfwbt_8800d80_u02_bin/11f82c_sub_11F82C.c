// v23 annotated: sub_11F82C @ 0x11f82c
// Original: 11f82c_sub_11F82C.c
// Primary struct: <unclustered>
//
// sub_11F82C @ 0x11f82c, size 412 bytes
// Doc: sub_121F82C [tx]: TX descriptor/buffer setup routine
// sub_121F82C [tx]: TX descriptor/buffer setup routine
int __fastcall sub_11F82C(int result)
{
  int v1; // r4
  bool *v2; // r2
  unsigned int v3; // r1
  _DWORD *v4; // r3
  void *v5; // r2
  _DWORD *v6; // r0
  int v7; // r1
  int *v8; // r6
  _DWORD *v9; // r5
  _DWORD *v10; // r7
  int v11; // r8
  _DWORD *v12; // r7
  int v13; // r10
  _DWORD *v14; // r9
  _DWORD *v15; // r8
  int v16; // r5
  int v17; // r3
  int v18; // r2
  _DWORD *v19; // r5
  int v20; // r2
  int v21; // r3
  _DWORD *v22; // r2
  int v23; // r1

  v1 = result;
  if ( *(_BYTE *)(result + 1224) )
    result = bt_rx_handler();
  if ( *(_DWORD *)(v1 + 72) )
    result = sub_1284C0(v1);
  if ( (*(_DWORD *)(v1 + 4) & 1) != 0 )
  {
    if ( (*(_DWORD *)off_11F9C8 & 0x2000000) != 0 || (result = *(_DWORD *)off_11F9C8 << 6, *((_BYTE *)off_11F9CC + 36)) )
    {
      result = feature_guard_sdio(2, dword_11F9D0);
      v2 = (bool *)off_11F9D4;
      v3 = *((unsigned __int8 *)off_11F9D4 + 1);
      *(_DWORD *)(v1 + 4) &= ~1u;
      *v2 = v3 > 1;
      if ( *((_BYTE *)off_11F9D8 + 190) )
      {
        if ( **(_BYTE **)off_11F9DC == 2 )
        {
          result = sub_117EF8();
          v4 = off_11F9E0;
          *(_DWORD *)off_11F9E0 &= ~1u;
          *v4 &= ~0x80u;
          if ( *(unsigned __int8 *)(v1 + 128) > 9u )
          {
            v5 = off_11F9C8;
            v6 = off_11F9E4;
            v7 = dword_11F9E8;
            *(_DWORD *)off_11F9C8 &= ~4u;
            if ( (*v6 & v7) == 0 )
              sub_1143D0((int)v6, v7, (int)v5);
            if ( *(_DWORD *)off_11F9EC << 28 )
            {
              if ( (__get_CPSR() & 1) == 0 )
              {
                __disable_irq();
                *(_DWORD *)off_11F9F0 = 1;
              }
              v8 = (int *)off_11F9F4;
              v9 = off_11F9EC;
              v10 = off_11F9F8;
              ++*(_DWORD *)off_11F9F4;
              *v9 = 0;
              v11 = v10[4];
              while ( *v9 << 28 )
              {
                if ( (unsigned int)(v10[4] - v11) > 0x7530 )
                  feature_guard_sdio(2, dword_11FA1C);
              }
              v12 = off_11F9F8;
              v13 = *((_DWORD *)off_11F9F8 + 4);
              if ( (*(_DWORD *)off_11F9FC & *(_DWORD *)off_11FA00 & 4) == 0 )
              {
                v14 = off_11F9FC;
                v15 = off_11FA00;
                do
                {
                  v16 = *v14 & *v15;
                  if ( (unsigned int)(v12[4] - v13) > 0x7530 )
                    feature_guard_sdio(2, dword_11FA04);
                }
                while ( (v16 & 4) == 0 );
              }
              *(_DWORD *)off_11FA08 = 4;
              if ( *v8 )
              {
                v17 = *v8 - 1;
                v18 = *(_DWORD *)off_11F9F0;
                *v8 = v17;
                if ( !v17 )
                {
                  if ( v18 )
                    __enable_irq();
                }
              }
            }
            v19 = off_11FA10;
            v20 = **(__int16 **)off_11FA0C;
            v21 = *((_DWORD *)off_11FA10 + 1) | 0x200;
            *((_DWORD *)off_11FA10 + 1) = v21;
            if ( v20 < 0 )
            {
              if ( *(_DWORD *)off_11F9EC << 28 )
              {
                sub_12F6C4(dword_11FA24, dword_11FA20, 472);
                v21 = v19[1];
              }
            }
            v22 = off_11F9EC;
            v23 = dword_11FA18;
            *(_DWORD *)off_11FA14 = v21 | *v19;
            *v22 = 48;
            return rf_table_lookup_n528(*(unsigned __int8 *)(v1 + 107), v23, v1);
          }
        }
      }
    }
  }
  return result;
}

