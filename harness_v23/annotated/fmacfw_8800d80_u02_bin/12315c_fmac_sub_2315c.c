// v23 annotated: fmac_sub_2315c @ 0x12315c
// Original: 12315c_fmac_sub_2315c.c
// Primary struct: <unclustered>
//
// fmac_sub_2315c @ 0x12315c, size 50 bytes
// Doc: fmac_sub_2315c [util]: FMAC internal utility routine
// fmac_sub_2315c [util]: FMAC internal utility routine
int __fastcall fmac_sub_2315c(int a1, int a2, int a3, int a4)
{
  _BYTE *v4; // r4
  int v5; // r1
  int v6; // r3

  v4 = (_BYTE *)sub_12C92C(129, a4, a3, 64);
  v5 = dword_123190;
  *v4 = 0;
  sub_143770(v4 + 1, v5, 36);
  v6 = (unsigned __int8)(*v4 + 36);
  *v4 = v6;
  v4[v6 + 1] = 0;
  sdio_buffer_prepare_n_4e8(v4);
  return 0;
}

