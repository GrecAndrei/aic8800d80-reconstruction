// v23 annotated: mmio_reg_modify_bits @ 0x1178c8
// Original: 1178c8_mmio_reg_modify_bits.c
// Primary struct: <unclustered>
//
// mmio_reg_modify_bits @ 0x1178c8, size 76 bytes
// Doc: mmio_reg_modify_bits [mmio]: Read-modify-write MMIO register using mask 0xf0001f
// mmio_reg_modify_bits [mmio]: Read-modify-write MMIO register using mask 0xf0001f
unsigned int mmio_reg_modify_bits()
{
  int v0; // r4
  unsigned int result; // r0

  v0 = dword_117918 & *(_DWORD *)off_117914;
  if ( (*(_DWORD *)off_117914 & 0x10) != 0 )
  {
    if ( (*(_DWORD *)off_117914 & 0x1000) != 0 )
    {
      v0 |= 0x1000u;
      *((_DWORD *)off_117920 + 52) |= 4u;
    }
    sub_11C2F4();
  }
  result = v0 & 0xF00000;
  if ( (v0 & 0xF00000) != 0 )
    result = sub_11AC74((unsigned __int8)(11 - __clz(result)));
  *(_DWORD *)off_11791C = v0;
  return result;
}

